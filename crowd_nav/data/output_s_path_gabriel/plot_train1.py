...
with open(file_path, 'r') as file:
    ...
    for line in file:
        env_match_latest = re.search(r"running 1/2 episode, simulation environment: (\w+)", line)
...
print(len(combined_averages_latest['Circle_crossing']['Total_Reward']))
...
for categ in combined_averages_latest.keys():
    for reward in combined_averages_latest[categ].keys():
        ...
        reward_values.append(combined_averages_latest[categ][reward])
...
for i in range(len(reward_list)):
    ...
    fig, axs = plt.subplots(2)#plt.figure()
    axs[0].scatter(x_axis_values, reward_values[i], s=1)
    axs[1].plot(x_axis_values, reward_values[i])
    #axs[0].set_title(reward_list[i])# + (' imitation learning' if i%2 == 0 else ' test'))
    #axs[1].set_title(reward_list[i])# + (' imitation learning' if i%2 == 0 else ' test'))
    plt.suptitle(reward_list[i])
plt.show()
